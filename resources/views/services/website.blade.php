            ...
            <div class="col-md-8 ">
                @foreach($websites as $website)
                    <div class="web-item col-md-12">
                        <div class="row">
                        <div class="col-md-4 web-img" style="background-image: url({{Voyager::image($website->image)}});">
{{--                            <img src="{{Voyager::image($website->image)}}" alt="" height="200px" width="200px">--}}
                        </div>
                        <div class="web-info col-md-8">
                            <a href="{{route('service.show_detail',[$website->service_id,$website->slug])}}"><h3 class="web_pg">{{ $website->title }}</h3></a>
                            <div class="post-date">
                                <i class="fa fa-clock-o" aria-hidden="true"></i>
{{--                                {{$website->created_at->format('d/m/Y')}}--}}
{{--                                {{$website->created_at}}--}}
                                {{Carbon\Carbon::parse($website->created_at)->format('d-m-Y')}}
                            </div>
                            <p>{!!mb_strimwidth($website->meta_description, 0, 150, "...")!!}</p>
                        </div>
                        </div>
                    </div>
                @endforeach
